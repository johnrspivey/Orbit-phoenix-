...
const tools = {
  ...
  pm2_restart: async ({name}) => new Promise((resolve,reject)=>{
  ...
  github_read: async ({owner,repo,path})=>{
    try{
      const r=await axios.get("https://api.github.com/repos/"+owner+"/"+repo+"/contents/"+path,{headers:{Authorization:"token "+GITHUB_TOKEN,Accept:"application/vnd.github.v3+json"}});
  ...
  github_write: async ({owner,repo,path,content,message,sha})=>{
    try{
      ...
      const r=await axios.put("https://api.github.com/repos/"+owner+"/"+repo+"/contents/"+path,payload,{headers:{Authorization:"token "+GITHUB_TOKEN,Accept:"application/vnd.github.v3+json"}});
