...
## The Big Picture
Sentinel is a proof of concept for physical Skipper.

Skipper is an AI command layer that orchestrates digital tools — GitHub, PM2, Netlify, Stripe. MCP is the protocol that gives it hands. Sentinel extends that same pattern into the physical world. The drones are just the first peripheral — assets that receive instructions, execute in the real world, and report back.

Scale the pattern and you get:
- Skipper coordinates
- Drones patrol
- Robots fabricate
- Sensors monitor
...
Same architecture. Same protocol. Different endpoints. Instead of `github_write` and `pm2_restart` the tool calls become `deploy_unit` and `execute_pattern` and `report_position`.

Sentinel isn't just a security product. It's the first step toward AI coordinating physical infrastructure it didn't build and doesn't own — just commands.

---

## Concept
AI command layer that orchestrates any drone — regardless of capability — into a unified fleet with purpose. Sentinel doesn't require drones to be smart. It assigns them roles and coordinates their behavior. The intelligence lives in the system, not the hardware.
...
## Related
- Orbit / Skipper MCP (command layer — digital version of this same architecture)
